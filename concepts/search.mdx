...
The `searchScope` field controls which corpus the search runs against. There are four scope shapes.
...
### Team connections search

The `team_connections` scope searches the directories that have been granted to a specific team. The API resolves the team to its granted directories and then runs directory search — your key must be an organization API key that belongs to the team's organization.
...
Global search works with both personal and organization API keys. Directory-scoped search — including `team_connections`, `directory`, and `directories` scopes — requires an organization API key. Personal API keys are rejected for any scope that targets a private corpus.
