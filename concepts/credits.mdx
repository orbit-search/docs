...
If the search succeeds and returns results, a profile query completes against an existing profile, a profile read returns a profile, or a Deep Search run is accepted, the reservation is finalized and your balance decreases. If the search fails or returns no results (a `404` response for an empty result set), a profile query or profile read returns `developer_profile_not_found`, or Deep Search fails before a run is accepted, the reservation is cancelled and your balance is restored. Failed validation, authentication, rate-limit, and system errors also do not consume credits.
...
|---|---|
| Search (`POST /v2/developer/search`, `POST /v2/developer/search/sse`) and Profile query (`POST /v2/developer/profiles/:id/query`) | 60 requests per 60 seconds |
