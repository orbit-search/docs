...
Authenticated users can retrieve a usage summary — including the current period allowance, credits used, remaining credits, recent ledger entries, and per-key breakdowns — with:

```http
GET /v2/api-keys/usage
```

Developer API keys cannot call this endpoint. Use your normal user access token to check usage.
