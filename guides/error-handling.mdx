...
<AccordionGroup>
  ...
  <Accordion title="Request validation errors">
    ...
    | `developer_deep_search_field_unsupported` | Deep Search received a top-level body field other than `mode`, `email`, `phoneNumber`, `linkedinUrl`, `urls`, `usernames`, `partialProfile`, or `structuredIntent`. |
    | `developer_deep_search_input_required` | Deep Search did not receive a supported input. |
    ...
    | `developer_deep_search_field_invalid` | Deep Search received malformed `urls` or `usernames`. |
    | `developer_deep_search_email_only_unsupported` | Email-only Deep Search is not supported; provide `structuredIntent` or `linkedinUrl`. |
    ...
    | `developer_deep_search_partial_profile_invalid` | Deep Search received a non-boolean `partialProfile` value. |
    | `developer_deep_search_partial_profile_generation_only` | Deep Search received `partialProfile` in `candidate_discovery` mode. |
    ...
    const VALIDATION_ERRORS = new Set([
      ...
      "developer_deep_search_email_only_unsupported",
      "developer_deep_search_generation_input_required",
      "developer_deep_search_generation_minimum_path_required",
      "developer_deep_search_phone_name_required",
      "developer_deep_search_candidate_discovery_structured_intent_only",
      "developer_deep_search_partial_profile_invalid",
      "developer_deep_search_partial_profile_generation_only",
  ...
  <Accordion title="Credit and rate limit errors">
    ...
    | HTTP | Code | Description |
    |------|------|-------------|
    | 402 | `developer_api_credits_insufficient` | Your remaining credits are lower than the cost of the requested operation (2 credits for Search, 10 for Deep Search, 1 for profile reads by default). The response includes `requiredCredits` and `remainingCredits`. Top up your credit balance before retrying. |
  ...
  <Accordion title="Developer Deep Search errors">
    ...
    | HTTP | Code | Description |
    |------|------|-------------|
    | 400 | `developer_deep_search_field_unsupported` | The body includes a top-level field other than `mode`, `email`, `phoneNumber`, `linkedinUrl`, `urls`, `usernames`, `partialProfile`, or `structuredIntent`. |
    | 400 | `developer_deep_search_input_required` | The body is missing a supported input. |
    ...
    | 400 | `developer_deep_search_field_invalid` | `urls` or `usernames` is malformed. |
    | 400 | `developer_deep_search_email_only_unsupported` | Email-only Deep Search is not supported; provide `structuredIntent` or `linkedinUrl`. |
    | 400 | `developer_deep_search_generation_input_required` | Generation mode is missing a supported profile generation input. |
    | 400 | `developer_deep_search_generation_minimum_path_required` | Generation requires `structuredIntent`, `linkedinUrl`, `urls + structuredIntent.names[0]`, or `phoneNumber + structuredIntent.names[0]`. |
    | 400 | `developer_deep_search_phone_name_required` | `phoneNumber` was sent without `structuredIntent.names[0]`. |
    | 400 | `developer_deep_search_candidate_discovery_structured_intent_only` | `candidate_discovery` mode only supports `structuredIntent`. |
    | 400 | `developer_deep_search_partial_profile_invalid` | `partialProfile` is present but is not a boolean. |
    | 400 | `developer_deep_search_partial_profile_generation_only` | `partialProfile` was sent in `candidate_discovery` mode. |
...
| Status | Codes |
|--------|-------|
| 400 | `developer_query_required`, `developer_num_users_required`, `developer_num_users_limit_exceeded`, `developer_user_id_override_forbidden`, `developer_search_replay_unsupported`, `structured_intent_required`, `structured_intent_invalid`, `structured_intent_version_unsupported`, `structured_intent_entity_clauses_unsupported`, `structured_intent_server_owned_field`, `search_directory_cross_org_scope_unsupported`, `developer_deep_search_field_unsupported`, `developer_deep_search_input_required`, `developer_deep_search_email_invalid`, `developer_deep_search_phone_invalid`, `developer_deep_search_linkedin_invalid`, `developer_deep_search_field_invalid`, `developer_deep_search_email_only_unsupported`, `developer_deep_search_generation_input_required`, `developer_deep_search_generation_minimum_path_required`, `developer_deep_search_phone_name_required`, `developer_deep_search_candidate_discovery_structured_intent_only`, `developer_deep_search_partial_profile_invalid`, `developer_deep_search_partial_profile_generation_only`, `developer_deep_search_name_required`, `developer_deep_search_arbitrary_input_required`, `developer_deep_search_personalization_unsupported` |
| 401 | `missing_api_key` |
