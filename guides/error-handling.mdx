...
<AccordionGroup>
  ...
  <Accordion title="Developer Deep Search errors">
    ...
    | HTTP | Code | Description |
    |------|------|-------------|
    | 400 | `developer_deep_search_name_required` | The request is missing `name`. |
    | 400 | `developer_deep_search_arbitrary_input_required` | An arbitrary discovery request is missing `search_terms`, `age`, and `age_range`. |
    | 400 | `developer_deep_search_invalid_web_url` | `web_url` is not a valid `http` or `https` URL. |
