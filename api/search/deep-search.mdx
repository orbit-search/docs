...
Developer Deep Search starts an asynchronous profile generation run from one or more identity signals — a LinkedIn URL, email, phone number, public URLs, usernames, or a [structured search intent](/api/search/structured-dsl). Depending on the run shape you choose, results can be delivered partial-first: a fast, source-backed partial profile can become available before full generation finishes.

You start a run with `POST /v2/developer/deep-search` and poll `GET /v2/developer/deep-search/{requestId}` until the run completes. There is no streaming endpoint; the previous SSE events route has been removed.
...
| Run shape | What you get | When to use it |
|---|---|---|
...
<ResponseField name="result" type="object | null" required>
  `null` until the run completes. On completion it contains the generated profile data — currently the Orbit profile id as `profile_id`. Read the generated profile with `GET /v2/developer/profiles/:id`.
</ResponseField>
