---
title: "POST /v2/developer/deep-search - generate profiles"
sidebarTitle: "Deep Search"
description: "Start an async Deep Search run from a developer API key, then poll the status endpoint until the run completes."
---
...
You start a run with `POST /v2/developer/deep-search` and poll `GET /v2/developer/deep-search/{requestId}` until the run completes. The status response returns the durable `profile_id` only; fetch the generated profile body separately with `GET /v2/developer/profiles/:id`. There is no streaming endpoint; the previous SSE events route has been removed.
...
```http
GET /v2/developer/deep-search/{requestId}
```

Poll the status endpoint until `status` is `completed` or `failed`. A few seconds between polls with backoff is appropriate; status reads share the Deep Search rate-limit bucket.
...
<ResponseField name="result" type="object | null" required>
  `null` until the run completes. On completion it contains the durable Orbit profile id as `profile_id` only. Read the generated profile body with `GET /v2/developer/profiles/:id`.
</ResponseField>

Requests for an unknown `requestId` return `404` with code `developer_deep_search_run_not_found`.
...
<AccordionGroup>
  ...
  <Accordion title="404 - Not found">
    | Code | Description |
    |---|---|
    | `developer_deep_search_run_not_found` | The `requestId` does not match a known run |
  </Accordion>
...
Start a run from a LinkedIn seed with an email signal, requesting a fast partial followed by full generation:
...
```bash curl
curl "$API_BASE/v2/developer/deep-search/8f6f2a1e-4b6d-4f3a-9d2c-1a7e5c3b9f10" \
  -H "Authorization: Bearer sk_orb_REDACTED"
```
...
do {
  ...
  const response = await fetch(
    `${API_BASE}/v2/developer/deep-search/${requestId}`,
...
while True:
    response = requests.get(
        f"{API_BASE}/v2/developer/deep-search/{request_id}",
        headers={"Authorization": "Bearer sk_orb_REDACTED"},
    )
