---
title: "POST /v2/developer/profiles/:id/query - profile query"
sidebarTitle: "Profile query"
...
```http
POST /v2/developer/profiles/:id/query
```
...
<ParamField path="id" type="string" required>
  The profile ID to search. This is the `id` value returned in the `payload.users` array from a [Search](/api/search/smart-search) response. You can also pass the public `orbitId` from a [Profile read](/api/search/profile-read) response.
</ParamField>
...
```bash
curl -X POST "$API_BASE/v2/developer/profiles/usr_a1b2c3d4e5f6/query" \
    ...
    <ResponseField name="payload.profile" type="object" required>
      ...
      <Expandable title="properties">
        <ResponseField name="payload.profile.id" type="string" required>
          The public profile identifier returned by the API. This is the `orbitId` when available. If the profile does not have an `orbitId`, this falls back to the profile identifier you supplied in the path and can be used again as `:id` for Profile Query or Profile Read.
        </ResponseField>
        <ResponseField name="payload.profile.orbitId" type="string">
          The stable Orbit profile identifier. Present when the profile has an Orbit ID. Prefer this value for caching and cross-surface correlation when it is present.
        </ResponseField>
...
The example below shows the full search-to-profile-query flow: run a search, extract the first profile ID from the response, then search inside that profile.
...
curl -X POST "$API_BASE/v2/developer/profiles/$PROFILE_ID/query" \
  -H "Authorization: Bearer sk_orb_REDACTED" \
...
// Step 1: run a search
const searchResponse = await fetch(`${API_BASE}/v2/developer/search`, {
  method: "POST",
  headers: {
...
const profileQueryResponse = await fetch(
  `${API_BASE}/v2/developer/profiles/${profileId}/query`,
  {
    method: "POST",
    headers: {
      "Authorization": "Bearer sk_orb_REDACTED",
...
profile_query_response = requests.post(
    f"{API_BASE}/v2/developer/profiles/{profile_id}/query",
    headers={
        "Authorization": "Bearer sk_orb_REDACTED",
