...
reads. If the requested ID is an alias for an existing identity, Orbit resolves
the alias and returns the canonical public profile.

This means the ID in the request path and the identifiers in the response may
not be identical. Treat `payload.orbitId` as the canonical Orbit profile ID after
every successful read. If you store profile references, update them to the
returned `payload.orbitId` so later reads target the canonical identity.
