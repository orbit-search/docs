...
On completion the result contains the durable `profile_id` only. Read the generated profile body with [`GET /v2/developer/profiles/{id}`](/api/search/profile-read).
...
<ResponseField name="result" type="object | null" required>
  `null` until the run completes. On completion it contains the durable Orbit profile id as `profile_id` only. Read the generated profile body with [`GET /v2/developer/profiles/{id}`](/api/search/profile-read).
</ResponseField>
