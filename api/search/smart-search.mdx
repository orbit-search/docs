...
<ResponseField name="payload" type="object" required>
  <Expandable title="properties" defaultOpen>
    <ResponseField name="payload.users" type="object[]" required>
      Array of matched profile objects, ranked by relevance. Each object includes at minimum an `id` field — the Orbit profile ID, the canonical identifier to pass to the [Profile Read](/api/search/profile-read) endpoint. Where present, `orbitId` carries the same value while older integrations migrate to Orbit profile IDs. The array is empty when no profiles matched.
    </ResponseField>
  </Expandable>
</ResponseField>
