...
description: "Run a natural language smart search against Orbit's people index. Returns ranked profiles. Supports global, team, and directory scopes. Costs 2 credits per result."
---
...
<ParamField body="searchScope" type="object">
  Scope of the search. Omit this field or set `{"type":"global"}` to search the full Orbit index. Use a scoped value to restrict results to a team's connections or one or more directories.

  <Expandable title="properties">
    <ParamField body="searchScope.type" type="string" required>
      One of: `"global"`, `"team_connections"`, `"directory"`, `"directories"`
    </ParamField>
    <ParamField body="searchScope.teamId" type="string">
      Required when `type` is `"team_connections"`. The UUID of the team whose directory grants are searched.
    </ParamField>
...
<Note>
  ...
  - **Personal API keys can only use global search.** To use `team_connections` or any directory scope, you must use an organization API key.
</Note>
...
<Tabs>
  ...
  <Tab title="Team connections">
    Search across all directories granted to a specific team. Requires an organization API key.
...
<AccordionGroup>
  ...
  <Accordion title="403 — Forbidden">
    ...
    | `developer_api_key_organization_required` | A directory or team scope was requested, but the key is a personal API key |
    ...
    | `developer_api_key_team_forbidden` | The organization API key is not authorized for the requested team |
