...
| Deep Search | `email` or `linkedinUrl`, or `structuredIntent.version`, `structuredIntent.names[0]`, and at least one discovery signal | Send either profile generation seed(s) or `structuredIntent`. Legacy top-level fields are rejected. |
...
Deep Search accepts `email` and/or `linkedinUrl` seeds for direct profile generation, or the same `structuredIntent` object on `POST /v2/developer/deep-search` when you are using arbitrary discovery. Send exactly one request shape; combining seed fields with `structuredIntent` returns `developer_deep_search_input_conflict`.
...
Deep Search requires `email`, `linkedinUrl`, or `structuredIntent`; legacy top-level fields such as `name`, `web_url`, `search_terms`, `age`, `age_range`, and location fields are not accepted on that endpoint.
...
The first `initial` event contains the same user payload returned by synchronous smart search. If `includeMatchReason` is `true`, additional `update` events can add match reasons for top results.
