...
Deep Search requires `email`, `linkedinUrl`, or `structuredIntent`; legacy top-level fields such as `name`, `web_url`, `search_terms`, `age`, `age_range`, and location fields are not accepted on that endpoint. Use `partialProfile: true` only with email or LinkedIn generation seed requests, not with `structuredIntent` arbitrary discovery.
...
When both are set, Search treats them as one semantic group: `text` is the canonical phrase, and `anyOf` adds alternatives for the same clause. Deep Search translates both the `text` phrase and each `anyOf` phrase into its internal arbitrary discovery terms.
